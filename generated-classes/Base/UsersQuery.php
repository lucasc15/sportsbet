 ...
 * @method     ChildUsersQuery orderByCorrect($order = Criteria::ASC) Order by the correct column
 * @method     ChildUsersQuery orderByIncorrect($order = Criteria::ASC) Order by the incorrect column
 *
 ...
 * @method     ChildUsersQuery groupByCorrect() Group by the correct column
 * @method     ChildUsersQuery groupByIncorrect() Group by the incorrect column
 *
 ...
 * @method     ChildUsers findOneByCorrect(int $correct) Return the first ChildUsers filtered by the correct column
 * @method     ChildUsers findOneByIncorrect(int $incorrect) Return the first ChildUsers filtered by the incorrect column *
 ...
 * @method     ChildUsers requireOneByCorrect(int $correct) Return the first ChildUsers filtered by the correct column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildUsers requireOneByIncorrect(int $incorrect) Return the first ChildUsers filtered by the incorrect column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUsers[]|ObjectCollection findByCorrect(int $correct) Return ChildUsers objects filtered by the correct column
 * @method     ChildUsers[]|ObjectCollection findByIncorrect(int $incorrect) Return ChildUsers objects filtered by the incorrect column
...
abstract class UsersQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'sportsbet', $modelName = '\\Users', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT userID, username, email, password, correct, incorrect FROM users WHERE userID = :p0';
        try {
    ...
    public function filterByCorrect($correct = null, $comparison = null)
    {
        if (is_array($correct)) {
            ...
            if (isset($correct['min'])) {
                $this->addUsingAlias(UsersTableMap::COL_CORRECT, $correct['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($correct['max'])) {
                $this->addUsingAlias(UsersTableMap::COL_CORRECT, $correct['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UsersTableMap::COL_CORRECT, $correct, $comparison);
    }

    /**
     * Filter the query on the incorrect column
     *
     * Example usage:
     * <code>
     * $query->filterByIncorrect(1234); // WHERE incorrect = 1234
     * $query->filterByIncorrect(array(12, 34)); // WHERE incorrect IN (12, 34)
     * $query->filterByIncorrect(array('min' => 12)); // WHERE incorrect > 12
     * </code>
     *
     * @param     mixed $incorrect The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return $this|ChildUsersQuery The current query, for fluid interface
     */
    public function filterByIncorrect($incorrect = null, $comparison = null)
    {
        if (is_array($incorrect)) {
            $useMinMax = false;
            if (isset($incorrect['min'])) {
                $this->addUsingAlias(UsersTableMap::COL_INCORRECT, $incorrect['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($incorrect['max'])) {
                $this->addUsingAlias(UsersTableMap::COL_INCORRECT, $incorrect['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UsersTableMap::COL_INCORRECT, $incorrect, $comparison);
    }
    ...
    public function filterByVotes($votes, $comparison = null)
    {
        if ($votes instanceof \Votes) {
            return $this
                ->addUsingAlias(UsersTableMap::COL_USERID, $votes->getUserid(), $comparison);
